const practicalLessons = [
  ...
  },
  {
    ...
    intro:
      "Callbacks are used for events, timers, array methods, and async work. You pass a function value so another function can call it later.",
    code: ["function run(callback) {", "  callback();", "}", "const sayReady = () => {", '  console.log("Ready");', "};", "run(sayReady);"],
    legend: ["variable", "object", "value", "wire"],
    nodes: {
      run: { label: "run", kind: "variable-wide", x: 16, y: 30 },
      runFn: { label: "fn", kind: "object", x: 42, y: 30 },
      sayReady: { label: "sayReady", kind: "variable-wide", x: 16, y: 54 },
      callbackValue: { label: "=> fn", kind: "object", x: 42, y: 54 },
      callback: { label: "callback", kind: "variable-wide", x: 42, y: 76 },
      ready: { label: '"Ready"', kind: "string", x: 70, y: 76 },
    },
    steps: [
      {
        title: "Create run",
      ...
      },
      {
        ...
        description: "() => { ... } creates a function value. () means it takes no inputs, and => points to the body that will run later.",
        line: 3,
        visible: ["run", "runFn", "sayReady", "callbackValue"],
        wires: [
          { id: "run-runFn", from: "run", to: "runFn", tone: "slate", fromAnchor: { side: "right" }, toAnchor: { side: "left" } },
          { id: "sayReady-callbackValue", from: "sayReady", to: "callbackValue", tone: "orange", fromAnchor: { side: "right" }, toAnchor: { side: "left" } },
        ],
        active: ["sayReady", "callbackValue"],
      },
      {
        title: "Pass sayReady into run",
        description: "run(sayReady) reads the function value from sayReady and passes that value as the argument. It still has not run yet.",
        line: 6,
        visible: ["run", "runFn", "sayReady", "callbackValue"],
        wires: [
          { id: "run-runFn", from: "run", to: "runFn", tone: "slate", fromAnchor: { side: "right" }, toAnchor: { side: "left" } },
      ...
      },
      {
        ...
        line: 0,
        visible: ["run", "runFn", "sayReady", "callbackValue", "callback"],
        wires: [
          { id: "run-runFn", from: "run", to: "runFn", tone: "slate", fromAnchor: { side: "right" }, toAnchor: { side: "left" } },
          { id: "sayReady-callbackValue", from: "sayReady", to: "callbackValue", tone: "slate", fromAnchor: { side: "right" }, toAnchor: { side: "left" } },
      ...
      },
      {
        ...
        description: 'callback() is the moment the arrow function runs. Then console.log receives "Ready".',
        line: 1,
    ...
    quiz: {
      prompt: "What does => create here?",
      options: ["a function value", "a string value", "a CSS rule"],
      answer: "a function value",
      correct: "Correct. The arrow syntax creates a function value. It runs later when callback() calls it.",
      wrong: "Not quite. => creates a function value; it does not run the body by itself.",
    },
  },
  {
